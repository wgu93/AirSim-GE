...
namespace airlib
{
    ...
    public: //methods
        ...
        // Weibin
        virtual void simSetGroundEffect(int model) override
        {
            switch (model)
            {
                case 0:
                {
                    ground_effect_model_ = 0;
                    Utils::log("simSetGroundEffect: no ground effect"); 
                    break;
                }
                case 1:
                {
                    ground_effect_model_ = 1;
                    Utils::log("simSetGroundEffect: Cheeseman-Bennett ground effect model is taking effect"); 
                    break;
                }
                case 2:
                {
                    ground_effect_model_ = 2;
                    Utils::log("simSetGroundEffect: Conyers's parametric ground effect model is taking effect"); 
                    break;
                }
            }
        }
    ...
    private: //methods
        ...
        void processMavMessages(const mavlinkcom::MavLinkMessage& msg)
        {
            ...
            else if (msg.msgid == HilControlsMessage.msgid) { // Sent from autopilot to simulation
                // Utils::log("msg.msgid == HilControlsMessage.msgid"); // print 'HilActuatorControlsMessage' when launched with PX4
            ...
            else if (msg.msgid == HilActuatorControlsMessage.msgid) { // Sent from autopilot to simulation
                // Utils::log("msg.msgid == HilActuatorControlsMessage.msgid"); // print this when launched with PX4
