...
namespace airlib
{
    ...
    protected:
        virtual void setWrench(Wrench& wrench) override
        {
            ...
            // Weibin
            // Utils::log("=============================================");
            // Utils::log(Utils::stringf("Wrench force x = %s [N]", std::to_string(wrench.force.x()).c_str()));
            // Utils::log(Utils::stringf("Wrench force y = %s [N]", std::to_string(wrench.force.y()).c_str()));
            // Utils::log(Utils::stringf("Wrench force z = %s [N]", std::to_string(wrench.force.z()).c_str()));
            // Utils::log("=============================================");
        }
    ...
    private: //methods
        static void setOutput(Output& output, const RotorParams& params, const FirstOrderFilter<real_T>& control_signal_filter, RotorTurningDirection turning_direction)
        {
            ...
            // Weibin
            // Utils::log("=============================================");
            // Utils::log(Utils::stringf("control_signal_filtered = %s", std::to_string(output.control_signal_filtered).c_str()));
            // Utils::log(Utils::stringf("max_speed_square = %s", std::to_string(params.max_speed_square).c_str()));
            // Utils::log(Utils::stringf("speed = %s", std::to_string(output.speed).c_str()));
            // Utils::log(Utils::stringf("thrust = %s", std::to_string(output.thrust).c_str()));
            // Utils::log(Utils::stringf("C_T = %s", std::to_string(params.C_T).c_str()));
            // Utils::log(Utils::stringf("max_rpm = %s [rev/min]", std::to_string(params.max_rpm).c_str()));
            // Utils::log("=============================================");
        }
